...
router.post("/admin/auth/forgot-password", async (req, res) => {
  ...
  console.log(`\n[LOCAL DEV] Forgot Password OTP for ${email}: ${otp}\n`);
  try {
    if (emailUser && emailPass !== "admin123") {
      ...
      console.warn("Skipped sending email due to default credentials.");
    }
    return res.json({ message: "OTP sent to your email (or terminal)" });
  } catch (err) {
    console.error("Email error:", err);
    // Bypassing 500 error for local dev
    return res.json({ message: "OTP printed to terminal (email failed)" });
  }
});
...
router.post("/admin/auth/send-signup-otp", async (req, res) => {
  ...
  console.log(`\n[LOCAL DEV] Signup OTP for ${email}: ${otp}\n`);
  try {
    if (emailUser && emailPass !== "admin123") {
      await transporter.sendMail({
        from: `"GymAdmin" <${emailUser}>`,
    ...
    return res.json({ message: "Verification OTP sent (check terminal)" });
  } catch (err) {
